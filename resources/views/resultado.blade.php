...
<div class="min-h-screen bg-gradient-to-b from-black via-[#0a0e27] to-black py-12 px-6 relative overflow-hidden">
    <!-- Animated Background -->
    ...
    <div class="relative z-10 max-w-6xl mx-auto">
        ...
        <div class="text-center mb-12 mission-complete-animation">
            <div class="inline-block mb-6">
                <div class="text-[#0667DA] text-sm font-bold tracking-[0.3em] uppercase mb-2">Missão Concluída</div>
                <h1 class="text-6xl md:text-7xl font-black text-white mb-3 tracking-tight">
                    @if($simulacao->score >= 80)
            ...
            <p class="text-xl text-blue-200">{{ $simulacao->vendedor_nome }} <span class="text-gray-500">vs</span> {{ ucfirst(str_replace('_', ' ', $simulacao->persona)) }}</p>
        </div>

        <!-- Score Display - Large Central -->
        <div class="mb-12 score-reveal-animation">
            <div class="relative max-w-md mx-auto">
                <!-- Glow Effect -->
                <div class="absolute inset-0 bg-gradient-to-br from-[#0667DA]/30 to-transparent blur-3xl"></div>
                
                <!-- Score Card -->
                <div class="relative bg-gradient-to-br from-[#0d1235] to-[#0a0e27] border-2 border-[#0667DA] rounded-3xl p-12 text-center">
                    <div class="text-[#0667DA] text-sm font-bold uppercase tracking-wider mb-4">Pontuação Final</div>
                    
                    <!-- Circular Progress -->
                    <div class="relative w-64 h-64 mx-auto mb-6">
                        <svg class="transform -rotate-90 w-64 h-64">
                            <circle cx="128" cy="128" r="120" stroke="rgba(6, 103, 218, 0.2)" stroke-width="12" fill="none" />
                            <circle cx="128" cy="128" r="120" 
                                ...
                                class="score-circle-animation" />
                        </svg>
                        <div class="absolute inset-0 flex items-center justify-center">
                            <div>
                                <div class="text-8xl font-black text-white score-number-animation">{{ $simulacao->score }}</div>
                                <div class="text-2xl text-gray-400 font-bold">/100</div>
                            </div>
                        </div>
                    </div>

                    <!-- Rank Badge -->
                    <div class="inline-flex items-center space-x-3 px-6 py-3 rounded-full text-xl font-black
        ...
        <div class="grid md:grid-cols-2 gap-6 mb-12">
            ...
            <div class="feedback-card-animation" style="animation-delay: 0.2s">
                <div class="bg-gradient-to-br from-green-900/30 to-[#0a0e27] border-2 border-green-500/50 rounded-2xl p-8 h-full">
                    <div class="flex items-center space-x-3 mb-6">
                        ...
                        <h3 class="text-2xl font-black text-green-400 uppercase tracking-tight">Acertos</h3>
                    </div>
            ...
            <div class="feedback-card-animation" style="animation-delay: 0.4s">
                <div class="bg-gradient-to-br from-blue-900/30 to-[#0a0e27] border-2 border-[#0667DA]/50 rounded-2xl p-8 h-full">
                    <div class="flex items-center space-x-3 mb-6">
                        <div class="w-12 h-12 bg-[#0667DA]/20 rounded-xl flex items-center justify-center">
                            <svg class="w-7 h-7 text-[#0667DA]" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>
                            </svg>
                        </div>
                        <h3 class="text-2xl font-black text-[#0667DA] uppercase tracking-tight">Melhorias</h3>
                    </div>
                    <div class="text-gray-300 leading-relaxed text-base">
        ...
        <div class="grid md:grid-cols-3 gap-6 mb-12">
            <div class="stat-card-animation" style="animation-delay: 0.6s">
                <div class="bg-[#0d1235] border border-[#0667DA]/30 rounded-xl p-6 text-center">
                    <div class="text-[#0667DA] text-xs font-bold uppercase tracking-wider mb-2">Duração</div>
                    <div class="text-4xl font-black text-white mb-1">{{ gmdate('i:s', $simulacao->duracao_segundos ?? 0) }}</div>
                    <div class="text-sm text-gray-500">minutos</div>
                </div>
            </div>
            <div class="stat-card-animation" style="animation-delay: 0.7s">
                <div class="bg-[#0d1235] border border-[#0667DA]/30 rounded-xl p-6 text-center">
                    <div class="text-[#0667DA] text-xs font-bold uppercase tracking-wider mb-2">Data</div>
                    <div class="text-4xl font-black text-white mb-1">{{ $simulacao->created_at->format('d/m') }}</div>
                    <div class="text-sm text-gray-500">{{ $simulacao->created_at->format('Y') }}</div>
                </div>
            </div>
            <div class="stat-card-animation" style="animation-delay: 0.8s">
                <div class="bg-[#0d1235] border border-[#0667DA]/30 rounded-xl p-6 text-center">
                    <div class="text-[#0667DA] text-xs font-bold uppercase tracking-wider mb-2">Horário</div>
                    <div class="text-4xl font-black text-white mb-1">{{ $simulacao->created_at->timezone('America/Sao_Paulo')->format('H:i') }}</div>
                    <div class="text-sm text-gray-500">Brasília</div>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-wrap items-center justify-center gap-4">
            <a href="{{ route('selecionar') }}" class="action-btn action-btn-primary">
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 5V1L7 6l5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6H4c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z"/>
                </svg>
                <span class="font-black uppercase tracking-wider">Nova Missão</span>
            </a>
            <a href="{{ route('historico') }}" class="action-btn action-btn-secondary">
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/>
                </svg>
                <span class="font-black uppercase tracking-wider">Ver Histórico</span>
            </a>
            <a href="{{ route('home') }}" class="action-btn action-btn-tertiary">
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/>
                </svg>
                <span class="font-black uppercase tracking-wider">Início</span>
            </a>
        </div>
    </div>
</div>
